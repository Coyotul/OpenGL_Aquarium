...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		float sharkRadius = 50.0f;
		float sharkangle = glfwGetTime() * (angularSpeed - 0.2f);

		// Calculăm noile coordonate x și z ale poziției peștelui pe baza unghiului și razei
		float sharknewX = sharkRadius * cos(sharkangle);
		float sharknewZ = sharkRadius * sin(sharkangle);
		...
		goldFishModelMatrix = glm::rotate(goldFishModelMatrix, -glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
